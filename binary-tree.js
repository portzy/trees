...
class BinaryTree {
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length) {
      ...
      let isHigher = currentVal > lowerBound;
      let shouldUpdate = currentVal < closest || closest === null;
  
      if (isHigher && shouldUpdate) {
        closest = currentVal;
      }
  ...
  areCousins(node1, node2) {
    ...
    function checkCousin(
      ...
      if (currentNode.right) {
        checkCousin(nodeToFind, currentNode.right, level + 1, data);
      }
      return data;
    }

    let node1Info = checkCousin(node1, this.root);
    let node2Info = checkCousin(node2, this.root);
  ...
  static deserialize(stringTree) {
    ...
    function buildTree() {
      ...
      if (values.length) {
        ...
        // convert values back into numbers
